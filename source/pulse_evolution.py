...
class PulseEvolution(object):
	...
	def load_fiber_parameter(self,duty_ratio,pitch_LB, center_wl,beta_degree=14):
		...
		omiga = 2 * np.pi * self._c_um_s / wl
		beta = neff * omiga / self._c_um_s
		#dW = wl[1]-wl[0]
		#beta1 = np.gradient(beta, dW)
		#beta2 = np.gradient(beta1, dW)
		#beta3 = np.gradient(beta2, dW)
		#beta4 = np.gradient(beta3, dW)
		#D = -6 * np.pi/wl**2 * beta2

		center_wl_um=center_wl/1000 #um
		omiga0 = 2 * np.pi * self._c_um_s / center_wl_um
		coeff = np.polyfit(omiga - omiga0, beta * 1e6, beta_degree)
		...
		omega_over_c = 1884.9431e7 / (center_wl_um*1e3) / 3
		#fr = 0.18;                              % fractional Raman contribution
		#fr = 0.18849431
		#omega_over_c = fr / (center_wl_um*1e-6) / self._c_m_s
		area0 = area0 * 1e-12 # m^2
		gamma = n2 * omega_over_c / area0 #[1/W·m]
	...
	def init_fiber(self,length_mm=100,center_wavelength_nm=1030,pitch_um=(3.1,3.0,2.9),duty_ratio=0.53,betas=(0, 0, 0),gamma_W_m=1,gvd_units='ps^n/km',alpha_db_cm=0.0,beta_degree=14,use_pitch=True):
		...
		if use_pitch:
			...
			chunk_num = len(pitch_um)
			chunk_length = length_mm / chunk_num

			for now_pitch in pitch_um:
				pitch_LB = math.floor(now_pitch * 10) / 10
				pitch_UB = pitch_LB + 0.1
				b_LB, g0_LB = self.load_fiber_parameter(duty_ratio, pitch_LB, center_wavelength_nm, beta_degree)
				if (duty_ratio==0.53 and now_pitch==3.1) or (duty_ratio==0.9 and now_pitch==7):
					b_UB, g0_UB =b_LB, g0_LB
				else:
					b_UB, g0_UB = self.load_fiber_parameter(duty_ratio, pitch_UB, center_wavelength_nm, beta_degree)
				...
				fiber1.generate_fiber(chunk_length* 1e-3, center_wl_nm=center_wavelength_nm, betas=tuple(beta_n[1:]),
				                      gamma_W_m=g0, gvd_units=gvd_units, gain=-alpha)
				self.fibers.append(fiber1)
		else:
			fiber1 = pynlo.media.fibers.fiber.FiberInstance()
			fiber1.generate_fiber(length_mm * 1e-3, center_wl_nm=center_wavelength_nm, betas=betas,
			                      gamma_W_m=gamma_W_m, gvd_units=gvd_units, gain=-alpha)
			self.fibers.append(fiber1)
	...
	def plot_result(self,path):
		...
		zW = self.dB(np.transpose(AW)[:, (F > 0)])
		zT = self.dB(np.transpose(AT))
		...
		ax0.plot(pulse_out.F_THz, self.dB(pulse_out.AW), color='r',label='pulse_out')
		ax1.plot(pulse_out.T_ps, self.dB(pulse_out.AT), color='r',label='pulse_out')

		ax0.plot(pulse.F_THz, self.dB(pulse.AW), color='b',label='pulse_input')
		ax1.plot(pulse.T_ps, self.dB(pulse.AT), color='b',label='pulse_input')
